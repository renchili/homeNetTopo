...
This is the single source/test regression entrypoint. A stage reports PASS only
after it executes successfully. Native Xcode build and Location/CoreWLAN runtime
acceptance remain deployment/macOS checks; this script statically guards the
native project, privacy metadata, fixed build boundary, and cache contract.
"""
...
def load_metadata() -> dict:
    """Load compact product metadata and require cross-owner sections."""
...
def documentation_guards() -> None:
    """Require docs at security, native evidence, state, and deployment boundaries."""

    required: dict[str, tuple[str, ...]] = {
        "server.py": (
            "<module>", "ApiError", "PassiveParts", "NativeWiFiCacheState",
            "read_native_wifi_cache", "AppState", "AppState.native_wifi_state",
            "AppState.collect_passive_parts", "AppState.passive_refresh", "AppState.active_discover",
            "HomeNetTopoServer", "HomeNetTopoHandler", "HomeNetTopoHandler._validate_collection_headers",
        ...
        "homenettopo/commands.py": ("<module>", "CommandError", "CommandKind", "CommandSpec", "run_command"),
        "homenettopo/discovery.py": ("<module>", "ValidationError", "validate_phase_a", "validate_phase_b", "validate_active_hosts"),
        "homenettopo/models.py": ("<module>", "TopologySnapshot", "TopologySnapshot.validate", "TopologySnapshot.to_dict"),
    ...
    comments = {
        "web/core.mjs": ("Pure frontend state", "evidence-backed path", "camera"),
...
def native_source_guards() -> None:
    ...
    for marker in (
        "CWWiFiClient.shared()", "requestWhenInUseAuthorization()", ".ssid()", ".bssid()",
        ".rssiValue()", ".noiseMeasurement()", ".wlanChannel()", ".transmitRate()",
        '"wifi-current.json"', "schemaVersion: 1", "cacheInterval: TimeInterval = 5",
    ):
        if marker not in collector:
            raise RuntimeError(f"native CoreWLAN collector contract missing: {marker}")
    for marker in ("SMAppService.mainApp", ".register()", ".unregister()", "openSystemSettingsLoginItems"):
    ...
    if "CODE_SIGN_ENTITLEMENTS" in project:
        raise RuntimeError("native helper unexpectedly introduced an entitlements file contract")
...
def consistency_guards() -> None:
    ...
    required_paths = (
        ...
        "tests/frontend/core.test.mjs", *NATIVE_PATHS,
    )
    ...
    if "RFC 1918" not in metadata["network_scope"] or commands.RFC1918_RANGES != discovery.RFC1918_RANGES:
    ...
    media = parse_wifi_hardware_ports(
    ...
    profiler = parse_airport_json(json.dumps({
        "SPAirPortDataType": [{"spairport_airport_interfaces": [{
            ...
            "spairport_current_network_information": {
                "_name": "Profiler Wi-Fi", "spairport_bssid": "02:aa:bb:cc:dd:01"
            },
        }]}],
    }))[0]
    native_evidence = parse_native_wifi_json(json.dumps({
        "schema_version": 1,
        "collected_at": "2026-08-08T12:00:00Z",
        "authorization": "authorized",
        "wifi": {
            "interface": "en0", "ssid": "Native Wi-Fi", "bssid": "02:aa:bb:cc:dd:42",
            "hardware_mac_address": "02:00:00:00:20:99", "channel": "40",
            "rssi_dbm": -35, "noise_dbm": -90, "phy_mode": "802.11ax",
            "transmit_rate_mbps": 2401,
        },
    }))
    if native_evidence.fact is None:
    ...
    merged = merge_wireless_facts((media,), (profiler,), (native_evidence.fact,), (fallback,))[0]
    if (
        merged.bssid != "02:aa:bb:cc:dd:42"
        or merged.ssid != "Native Wi-Fi"
        or merged.hardware_mac_address != "02:00:00:00:20:01"
        or merged.role != "relay"
        or merged.evidence_source != "wifi_native"
    ...
    route = RouteFact("0.0.0.0/0", "192.168.1.1", ("U", "G"), "en0", True)
    snapshot = build_snapshot(
        ...
        sources=(
            ...
            SourceStatus("wifi_native", SourceStatusValue.OK),
        ),
        collected_at="2026-08-08T12:00:00Z",
    )
    ...
    if set(host.mac_addresses) != {"02:00:00:00:10:01", "02:00:00:00:20:01"}:
        raise RuntimeError("local private/hardware MAC identity is incomplete")
    ...
    for owner in ("agent", "api", "design", "readme"):
        ...
        for phrase in (
            "corewlan", "corelocation", "location", "hardware mac", "private wi-fi mac",
            "bssid", "gateway", "peer", "lldp", "rfc 1918", "networksetup",
            "concurrent", "launchagent", "xcode",
        ):
            if phrase not in lowered:
                raise RuntimeError(f"Wi-Fi/path/deployment contract mismatch in {owner}: missing {phrase}")

    server_markers = (
        "ThreadPoolExecutor", "read_native_wifi_cache", "wifi_native", "merge_wireless_facts",
        "corewlan_native_then_system_profiler", "homenettopo-wifi://authorize", '"timeout_sources"',
        "font-src 'self' data:",
    )
    ...
    for marker in ("parse_native_wifi_json", "evidence_source", "wifi_native", "hardware_mac_address", "bssid_observed"):
        if marker not in texts["interfaces"]:
            raise RuntimeError(f"Wi-Fi parser contract missing: {marker}")
    for marker in ("local_macs", "private_wifi_mac_address", "wifi_native", "identity_source", '"bssid"'):
    ...
    for marker in (
        ...
        "NATIVE_SOURCE_FILES", "validate_native_source_root", "build_native_app",
        "replace_native_app", "open_native_helper", "ProxyHandler({})",
    ...
    test_contracts = {
        "tests/test_interfaces.py": (
            "test_native_wifi_parser_keeps_current_corewlan_identity_and_metrics",
            "test_native_bssid_wins_profiler_while_user_confirmed_role_survives",
        ),
        "tests/test_topology.py": (
            "test_local_ip_and_local_macs_are_host_identity_not_peer_devices",
            "test_native_corewlan_bssid_is_high_confidence_current_wifi_node",
            "test_user_confirmed_relay_fallback_is_visible_without_claiming_observation",
        ),
        "tests/test_server.py": (
            "test_native_wifi_cache_requires_fresh_owned_regular_data",
            "test_native_wifi_identity_wins_profiler_and_local_fallback",
            "test_capabilities_explain_native_wifi_source_without_disclosing_identity",
        ),
        "tests/test_static_security.py": (
            "test_native_privacy_identity_and_login_item_sources_are_explicit",
...
def main() -> int:
    """Execute configured source/test stages and print one truthful summary."""

    parser = argparse.ArgumentParser()
    parser.add_argument("--python-only", action="store_true", help="Skip frontend Node tests; not full source/test regression evidence")
    ...
    if failed:
        return 1
    if args.python_only:
        print("Python-only development checks completed; full source/test regression was NOT RUN.")
    return 0
